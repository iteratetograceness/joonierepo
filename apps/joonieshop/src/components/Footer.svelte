...
<footer class="flex w-full items-center justify-between px-4 py-3">
	{#each links_and_text as link_or_text}
		{#if link_or_text.href}
			<a
				href={link_or_text.href}
				class="text-lg transition-[color] duration-200 hover:text-light-blue"
			>
				{link_or_text.text}
			</a>
		{:else}
